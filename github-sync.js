class GitHubPhotoSync {
    ...
    async upsertPhotosIssue(photos) {
        ...
        try {
            ...
            if (issueDataSize > maxSize) {
                throw new Error(`上传数据过大 (${this.formatBytes(issueDataSize)}), 超过了GitHub API限制 (${this.formatBytes(maxSize)})。请减少照片数量或使用更激进的压缩设置。`);
            }
